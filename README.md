...
### 3. 准备 Groq API Key（无字幕兜底）

当视频没有字幕或部分分 P 缺字幕时，程序会自动尝试音频转写方案。

你可以在软件内的“使用说明”里直接跳转到：

- Groq API Keys

拿到 Key 后，填到设置里的 `Groq API Key` 即可。

## 适用范围

知语狸不是“任何视频都能完美总结”。

它目前的工作方式是：**优先使用字幕读取；没有字幕时，再尝试音频转写**。  
所以它更适合：

- 本身有字幕的视频
- 有 AI 自动字幕的视频
- 口播清晰、背景噪音较少的视频
- 教程、知识、访谈、资讯这类信息密度较高的视频

## 你需要知道的限制

- 如果视频没有字幕，程序会尝试调用 Groq 做音频转写，再继续生成总结
- 如果多分 P 视频里只有部分分 P 能拿到字幕或转写文本，程序只会总结已获取文本的那部分
